...
- (IBAction)actionSaveHandler:(UIButton *)sender {
    ...
    //validations:
    UIAlertView *alertViewObject=[[UIAlertView alloc]initWithTitle:@"Error!!" message:nil delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles: nil];
    if(textFieldFirstName.text.length==0){
        alertViewObject.message=@"Kindly enter FirstName!!";
        [alertViewObject show];
    }
    else if(textFieldLastName.text.length==0){
        alertViewObject.message=@"Kindly enter LastName!!";
        [alertViewObject show];
        
    }
    else if (textFiledCity.text.length==0){
        alertViewObject.message=@"Kindly enter City!!";
        [alertViewObject show];
    }
    else if (textFiledState.text.length==0){
        alertViewObject.message=@"Kindly enter State!!";
        [alertViewObject show];
    }
